...
export default function AddProductPage() {
  ...
  const addBenefit = () => {
    const val = prompt("Enter benefit:")?.trim()
    if (val) setBenefits((p) => [...p, val])
  }

  const addTag = () => {
    const val = prompt("Enter tag:")?.trim()
    if (val) setTags((p) => [...p, val])
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const product = {
      ...
      flashSaleEnds: form.isFlashSale ? form.flashSaleEnds || undefined : undefined,
      tags,
    }

    try {
      await createProduct(product)
      dispatch({ type: "ADMIN_ADD_PRODUCT", payload: product })
      toast({ title: "Product Created", description: `${product.name} has been added.`, variant: "success" })
      router.push("/admin/products")
    } catch (err) {
      toast({ title: "Error", description: "Failed to create product.", variant: "error" })
    }
    setSaving(false)
  }
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <form onSubmit={handleSubmit} className="space-y-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-6 rounded-2xl border bg-card space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2 space-y-2">
                ...
                <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Vitamin C Brightening Serum" required />
              </div>
              <div className="space-y-2">
                <Label>Brand</Label>
                <Input value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} list="brands-list" placeholder="e.g. GlowLab" />
                <datalist id="brands-list">{brandOptions.map((b) => <option key={b} value={b} />)}</datalist>
              </div>
              <div className="space-y-2">
                <Label>Category</Label>
                <Input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} list="categories-list" placeholder="e.g. Skincare" />
                <datalist id="categories-list">{categoryOptions.map((c) => <option key={c} value={c} />)}</datalist>
              </div>
              <div className="space-y-2">
                <Label>Sub Category</Label>
                <Input value={form.subCategory} onChange={(e) => setForm({ ...form, subCategory: e.target.value })} placeholder="e.g. Serums" />
              </div>
              <div className="space-y-2">
                <Label>Product Type</Label>
                <Input value={form.productType} onChange={(e) => setForm({ ...form, productType: e.target.value })} list="ptypes-list" placeholder="e.g. Serum" />
            ...
            <div className="space-y-2">
              ...
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={4} className="w-full rounded-xl border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring resize-none" placeholder="Product description..." />
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="p-6 rounded-2xl border bg-card space-y-6">
            ...
            <div className="space-y-2">
              <Label>Product Images (upload from device)</Label>
            ...
            <div className="space-y-2">
              <Label>Videos (upload from device)</Label>
              ...
              {videos.length > 0 && (
                <div className="flex gap-2 flex-wrap mt-3">
                  {videos.map((src, i) => (
                    <div key={i} className="relative w-32 h-20 rounded-lg overflow-hidden group bg-muted">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="p-6 rounded-2xl border bg-card space-y-6">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              <div className="space-y-2"><Label>Selling Price ($) *</Label><Input type="number" step="0.01" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="38.00" required /></div>
              <div className="space-y-2"><Label>Original Price ($)</Label><Input type="number" step="0.01" value={form.originalPrice} onChange={(e) => setForm({ ...form, originalPrice: e.target.value })} placeholder="52.00" /></div>
              <div className="space-y-2"><Label>Stock Quantity</Label><Input type="number" value={form.stock} onChange={(e) => setForm({ ...form, stock: e.target.value })} placeholder="50" /></div>
            </div>
            {form.price && form.originalPrice && parseFloat(form.originalPrice) > parseFloat(form.price) && (
              <p className="text-sm text-emerald-600">Discount: {Math.round(((parseFloat(form.originalPrice) - parseFloat(form.price)) / parseFloat(form.originalPrice)) * 100)}% off</p>
            )}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="p-6 rounded-2xl border bg-card space-y-6">
            ...
            <div className="space-y-2"><Label>Ingredients</Label><textarea value={form.ingredients} onChange={(e) => setForm({ ...form, ingredients: e.target.value })} rows={3} className="w-full rounded-xl border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring resize-none" placeholder="Comma separated list of ingredients..." /></div>
            <div className="space-y-2"><Label>How to Use</Label><textarea value={form.howToUse} onChange={(e) => setForm({ ...form, howToUse: e.target.value })} rows={3} className="w-full rounded-xl border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring resize-none" placeholder="Usage instructions..." /></div>
            ...
            <div className="space-y-2">
              ...
              <Button type="button" variant="outline" size="sm" onClick={addBenefit}><Plus className="h-3 w-3" /> Add Benefit</Button>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Button type="button" variant="outline" size="sm" onClick={addTag}><Plus className="h-3 w-3" /> Add Tag</Button>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="p-6 rounded-2xl border bg-card space-y-6">
            ...
            {form.isFlashSale && (
              <div className="space-y-2">
                ...
                <Input type="datetime-local" value={form.flashSaleEnds} onChange={(e) => setForm({ ...form, flashSaleEnds: e.target.value })} />
              </div>
            )}
          </motion.div>
